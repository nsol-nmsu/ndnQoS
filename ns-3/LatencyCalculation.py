...
#               print(values)
                lineno=lineno+1
                if(values[5]=="TypeI") and float(values[3]) != SIMULATION_TIME:
                        count1=count1+1
                        type1lat=type1lat+float(values[4])
                if(values[5]=="TypeII") and float(values[3]) != SIMULATION_TIME:
                        count2=count2+1
                        type2lat=type2lat+float(values[4])
                if(values[5]=="TypeIII") and float(values[3]) != SIMULATION_TIME:
                        count3=count3+1
                        type3lat=type3lat+float(values[4])
        writer.writerow([type1lat/count1,type2lat/count2,type3lat/count3])
        #writer.writerow([type1lat/count1])
