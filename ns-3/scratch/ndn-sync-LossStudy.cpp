...
#include "ndn-sync-LossStudy.hpp"
namespace ns3 {
...
main( int argc, char* argv[] )
{
  ...
  cmd.AddValue("PortNum", "PortNum", portNum);
  cmd.Parse( argc, argv );

  sync.setPort(portNum);
  sync.setLoss(lossRate);
  ...
  bool attachingPMUs = false, attachingLCs = false, injectData = false, Sync = false;
  ...
  char trace[100];
  sprintf( trace, "ndn-case_LossStudy-run%d.csv", run );
