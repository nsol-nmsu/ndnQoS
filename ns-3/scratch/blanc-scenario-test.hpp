...
int main (int argc, char *argv[])
{
  ...
  std::ifstream configFile ("../topology/interface/blancPP-TestCase.txt", std::ios::in);
  ...
  unordered_map<int,double> costMap;
  unordered_map<int,int> neighborMap;  
  ...
  if (configFile.is_open ()) {
		...
		//	if(!HasIP(std::stoi(netParams[0]))){
		   ...
		   netParams = SplitString(strLine);
                   costMap[std::stoi(netParams[0])] = std::stod(netParams[2]);
                   neighborMap[std::stoi(netParams[0])] = std::stoi(netParams[1]); 
                   //sync.setNeighborCredit(std::stoi(netParams[0]), netParams[1], std::stod(netParams[2]) );
                   //sync.setNeighborCredit(std::stoi(netParams[1]), netParams[0], std::stod(netParams[2]) );
  ...
  for (auto it = neighborMap.begin(); it != neighborMap.end(); it++) {
        std::cout<<"Check this out..." <<it->first<<" "<<it->second<<std::endl;
        sync.setAddressTable(it->first, std::to_string(it->second), Ipv4Address(GetIP(it->second).c_str()) );
        sync.setAddressTable(it->second, std::to_string(it->first), Ipv4Address(GetIP(it->first).c_str()) );

        sync.setNeighborCredit(it->first, std::to_string(it->second), costMap[it->first] );
        sync.setNeighborCredit(it->second, std::to_string(it->first), costMap[it->first] );
...
void onTx(uint32_t node,  uint32_t txid){
   tracefile << txid<<", Start, "<<ns3::Simulator::Now().GetSeconds() << std::endl;
};
...
void onPay(uint32_t node,  uint32_t txid){
   tracefile << txid<<", Completed, "<<ns3::Simulator::Now().GetSeconds() << std::endl;
