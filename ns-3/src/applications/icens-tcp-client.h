...
#ifndef ICENS_TCP_CLIENT
#define ICENS_TCP_CLIENT
 ...
 */
class iCenSTCPClient : public Application
{
public:

  typedef void (* ReceivedPacketTraceCallback) (uint32_t nodeid, Ptr<Packet> packet, const Address &address);
  typedef void (* SentPacketTraceCallback) (uint32_t nodeid, Ptr<Packet> packet, const Address &address, uint32_t seqNo, uint32_t subscription);

  static TypeId GetTypeId (void);

  iCenSTCPClient ();

  virtual ~iCenSTCPClient ();
...
private:
  ...
  TracedCallback<uint32_t, Ptr<Packet>, const Address &, uint32_t, uint32_t> m_sentPacket;
